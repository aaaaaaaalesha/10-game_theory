...
class BimatrixGame:
    ...
    def __is_nash_optimal(self, first_index: IndexType, second_index: IndexType) -> bool:
        ...
        m, n, _ = self.bimatrix.shape
        first, second = self.bimatrix[first_index, second_index]
        if any(self.bimatrix[i][second_index][0] >= first for i in range(m) if i != first_index):
            return False

        return all(self.bimatrix[first_index][j][1] <= second for j in range(n) if j != second_index)
